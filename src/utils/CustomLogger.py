...
class CustomLogger(Process):
    ...
    def process(
        ...
        if not os.path.exists(LOCAL_DIR + "/log/"):
            os.makedirs(LOCAL_DIR + "/log/")
            if not os.path.exists(LOCAL_DIR + "/log/app.log"):
                open(LOCAL_DIR + "/log/app.log", "w").close()
            if not os.path.exists(LOCAL_DIR + "/log/error.log"):
                open(LOCAL_DIR + "/log/error.log", "w").close()

        app_logger = logging.getLogger("APP LOGGER")
        hdlr_2 = logging.FileHandler(LOCAL_DIR + "/log/app.log")
        ...
        hdlr_4 = logging.FileHandler(LOCAL_DIR + "/log/error.log")
